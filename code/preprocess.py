...
def preprocess(data_file, user_data_file):
	...
	with open(data_file) as f:
		...
		line = "aa"
		stopper = 0
		while line != "" and stopper < 1000000:
			...
			if event[0] not in user_events.keys():  # check if listener exists in events dictionary
				...
				values = np.array([timestamp, 0.0, artist_id, track_id])
				user_events[event[0]] = [values]
			else:
				last_listen = user_events[event[0]][-1][0]  # timestamp of last listen
				secs_since_last_listen = np.abs((timestamp - last_listen).total_seconds())

				values = np.asarray([timestamp, secs_since_last_listen, artist_id, track_id])
				user_events[event[0]].append(values)

	for k in user_events.keys():
		for item in user_events[k]:
			item[0] = (item[0] - datetime(1970,1,1)).total_seconds()
	...
	with open(user_data_file) as f:
		...
		while line != "" and stopper < 1000000:
			...
			# Map gender
			if len(user) > 1 and user[1] == 'f':
				gender = 0
			elif len(user) > 1 and user[1] == 'm':
				gender = 1
			else:  # Empty
				gender = 2
			...
			country_dict = {}
			country_id = 0
			if len(user) > 3 and user[3] not in country_dict.keys():
